...
-- Create cards table with UUID primary key
CREATE TABLE IF NOT EXISTS cards (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  card_image_id TEXT UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_cards_image_id ON cards(card_image_id);
...
DROP POLICY IF EXISTS "Allow public read access to sets" ON sets;
DROP POLICY IF EXISTS "Allow public read access to cards" ON cards;
DROP POLICY IF EXISTS "Allow service role to manage sets" ON sets;
DROP POLICY IF EXISTS "Allow service role to manage cards" ON cards;
